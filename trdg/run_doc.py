...
def parse_arguments():
    ...
    # Count (required unless -i is a directory)
    parser.add_argument(
        "-c", "--count", type=int, default=0,
        help="Number of document pages to generate. "
             "When -i is a directory, this is ignored (one page per file).",
    )
    ...
    parser.add_argument(
        ...
        help="Path to a .txt file or a directory of .txt files. "
             "File: generates one page from its content. "
             "Directory: generates one page per .txt file found.",
    )
    parser.add_argument(
